...
-->
<template>
    <div class="behavior-menu height100" v-loading="isLoading">
    ...
    export default {
        ...
        methods: {
            async reqDelete(params, isCascader) {
                const vm = this;
                vm.isLoading = true;
                let {data} = await vm.$axios.post('deleteBehavior',{
                    id: params.id,
            ...
            async reqTree() {
                ...
                if (data.code === 200) {
                    ...
                    vm.$message.error(data.msg || '查询领域行为列表异常');
                }
                vm.list = tree;
            },
            async init(){
            ...
            handleDelete(data) {
                const vm = this;
                vm.$confirm('此操作将永久删除该领域行为, 是否继续?', '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(async () => {
                    if (data.children && data.children.length > 0) {
                        vm.$confirm('是否删除本身及其子行为?', '选择', {
                            confirmButtonText: '仅本身',
                            cancelButtonText: '本身及子行为',
                            type: 'success ',
                            showClose: false
                        }).then(async () => {
