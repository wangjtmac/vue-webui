...
<template>
    <el-form class="domain-edit"
             ...
             v-loading="isLoading"
    >
        <el-form-item label="行为类型" prop="name">
    ...
    export default {
        ...
        data() {
            ...
            return {
                ...
                rules: {
                    name: [
                        { required: true, message: '请输入行为类型', trigger: 'blur' },
        ...
        methods: {
            async reqAdd(params) {
                const vm = this;
                vm.isLoading = true;
                let {data} = await vm.$axios.post('createBehavior',params).catch((error) => {
                ...
                if (data.code === 200) {
                    ...
                    vm.$message.error(data.msg || '新增领域行为异常');
                    return false;
                }
            },
            async reqUpdate(params) {
                const vm = this;
                vm.isLoading = true;
                let {data} = await vm.$axios.post('updateBehavior',params).catch((error) => {
                    return {data: {msg: '服务器异常，请联系管理员'}};
                });
                vm.isLoading = false;
                if (data.code === 200) {
                    ...
                    vm.$message.error(data.msg || '修改领域行为异常');
                    return false;
                }
            },
            /**
            ...
            async reqData(id) {
                const vm = this;
                vm.isLoading = true;
                let {data} = await vm.$axios.post('getBehaviorById',{id}).catch((error) => {
                    return {data: {msg: '服务器异常，请联系管理员'}};
                });
                vm.isLoading = false;
                if (data.code === 200) {
                    return data.domainBehavior;
                } else {
                    vm.$message.error(data.msg || '获取领域行为信息异常');
                    return {};
                }
            },
            async initForm(data) {
