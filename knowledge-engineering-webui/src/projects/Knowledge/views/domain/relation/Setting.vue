    ...
    export default {
        ...
        data() {
            return {
                ...
                form: {},
                columns: [
                    {label: '关系属性', prop: 'name', align: 'center'},
        ...
        methods: {
            async reqSave(params) {
                const vm = this;
                let {data} = await vm.$axios.post('saveRelationConfig', params).catch((error) => {
            ...
            async reqData(id) {
                const vm = this;
                let {data} = await vm.$axios.post('relationConfig', id).catch((error) => {
                    return {data: {msg: '服务器异常，请联系管理员'}};
                });
                if (data.code !== 200) {
                    vm.$message.error(data.msg || `查询关系配置数据异常`);
            ...
            async initData() {
                ...
                if (relation && relation.id) {
                    ...
                    if (!vm.relation || vm.relation.id !== id) { // 被篡位，则不交数据
                        return null;
                    }
                    vm.isLoading = false;
                }
            ...
            initForm(data = {}, index, flag) {
                ...
                let {form} = vm;
                let formItem = {};
                let filterList = ["relationId", "inName", "outName", "listColumn", "siteObjName", "siteObjColumn","featureDataTypeMap"];
            ...
            initList(data = {}, idx) {
                ...
                let listColumn = [];
                listColumn.push({
                    name: `左顶点(出)${form.outName ? ` - ${form.outName}` : ''}`,
                    prop: 'outColumn',
                    required: true,
                    rules: {
                        required: true, message: '左顶点(出)不能为空', trigger: 'blur'
                    },
                    model: data,
                    modelField: 'outColumn'
                });
                listColumn.push({
                    name: `右顶点(入)${form.inName ? ` - ${form.inName}` : ''}`,
                    prop: 'inColumn',
                    required: true,
                    rules: {
                        required: true, message: '右顶点(入)不能为空', trigger: 'blur'
                    },
                    model: data,
                    modelField: 'inColumn'
                });
                ...
                let filterList = ["id", "siteObjName", "siteColumn", "startTimeColumn", "endTimeColumn", "inColumn", "outColumn", "dataObjId", "dataSourceId","featureDataTypeMap"];
                let dataList = vm.copyAttributes(data, filterList);
